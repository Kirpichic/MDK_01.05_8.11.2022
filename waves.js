...
canvas.header = window.innerHeight;
var waves = ["rgba(157, 187, 210, 0.3)"]
var i = 0;
function draw(){
    canvas.width = canvas.width;
    var offset = i + i * Math.PI * 12;
    ctx.fillStyle = "#138CCB";
    var randomLeft = (Math.sin(offset/100)+1)/2*150;
    var randomRight = (Math.sin((offset/100)+10)+1)/2*150;
    var randomLeft = (Math.sin((offset/60)+2)+1)/2*150;
    var randomLeft = (Math.sin((offset/60)+1)+1)/2*150;
    ctx.beginPath();
    ctx.maveTo(0, randomLeft + 150);
